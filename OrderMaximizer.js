...
Orders should be matched and processed based on the following criteria:
    ...
    - Processing a matched pair of orders simply means using up available quantity and incrementing the total by the sale amount.
...
2. the remaining 80 sell quantities are matched with the next highest buy which is the leftover 90 price buys:
- total: 770 + (80 * 2) = 930
...
class OrderSaleMaximizer {
    ...
    // Matches highest buy to lowest sell and then processes when the sell price is less than or equal
    processMatchingOrders(){
        // Save some time complexity so we don't remove and re-insert orders unnecessarily
